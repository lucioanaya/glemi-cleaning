...
if(bookingForm){
  ...
  async function generateQuote(){if(!hasRooms()){toast('Agrega al menos un área para generar la cotización.');return}setStep(2);const loading=document.getElementById('aiLoading'),result=document.getElementById('aiResult');loading.classList.remove('hidden');result.classList.add('hidden');await new Promise(r=>setTimeout(r,500));const data=calculateQuote();state.quote=data;document.getElementById('aiTotal').textContent=`${money(data.total)} USD`;document.getElementById('aiMessage').textContent=data.message;const host=document.getElementById('aiBreakdown');host.innerHTML='';data.breakdown.forEach(line=>{const div=document.createElement('div');div.className='breakdown-line';div.innerHTML=`<span>${line.item}</span><strong>${money(line.amount)}</strong>`;host.appendChild(div)});if(data.addonsAmount>0){const div=document.createElement('div');div.className='breakdown-line addon-line';div.innerHTML=`<span>Servicios adicionales (${data.addons.length})</span><strong>+${money(data.addonsAmount)}</strong>`;host.appendChild(div)}loading.classList.add('hidden');result.classList.remove('hidden')}
